...
def get_course_list(degree_id, faculty):
    ...
    while True:
        url = (
            f"{baseUrl}?pv_num_pag={page}&pv_ano_lectivo={YEAR}&pv_curso_id={degree_id}"
        )
        response = requests.get(url)
        if response.status_code != 200:
            raise Exception(f"Error {response.status_code} fetching page")
        ...
        for row in rows:
            ...
            if len(aHtml) > 0:
                courses.append(
                    f"https://sigarra.up.pt/{faculty}/pt/"
                    + aHtml[0].get("href").strip()
                )
